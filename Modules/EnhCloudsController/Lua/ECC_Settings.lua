...
function ECC_Win_Settings()
    ...
    if ECC_Preferences.Window_Page == 1 then
		--[[ "Back" button ]]
        if imgui.Button("Main Menu",(ECC_Preferences.AAA_Window_W-15),20) then ECC_Preferences.Window_Page = 0 ECC_CheckAutosave() end
        ...
        imgui.Dummy((ECC_Preferences.AAA_Window_W-15),19)
        -- [[ Hotkey options ]]
        local changed, newWindowToggleHotkeyEnable = imgui.Checkbox("Toggle window by key##55", ECC_Preferences.AAA_WindowToggleHotkeyEnable)
        if changed then ECC_Preferences.AAA_WindowToggleHotkeyEnable = newWindowToggleHotkeyEnable ECC_CheckAutosave() end
        if ECC_Preferences.AAA_WindowToggleHotkeyEnable then imgui.SameLine()  imgui.TextUnformatted("(number):") imgui.SameLine()
            local changed, newWindowToggleHotkey = imgui.PushItemWidth(50) imgui.InputText("##56", ECC_Preferences.AAA_WindowToggleHotkey, 3) imgui.PopItemWidth() 
            if changed and newWindowToggleHotkey ~= "" and tonumber(newWindowToggleHotkey) then ECC_Preferences.AAA_WindowToggleHotkey = tonumber(newWindowToggleHotkey) ECC_CheckAutosave() end		
    ...
    --			end
        ...
        ECC_GetWindowInfo()
        imgui.TextUnformatted("Window Size and position data is saved automatically!")
        imgui.Dummy((ECC_Preferences.AAA_Window_W-15),19)
        -- [[ Preference file control buttons ]]
        if imgui.Button("Save Preferences",(ECC_Preferences.AAA_Window_W-15),20) then ECC_File_Write("PrefsFile") end
        if imgui.Button("Load Preferences",(ECC_Preferences.AAA_Window_W-15),20) then ECC_File_Read("PrefsFile") end
        imgui.Dummy((ECC_Preferences.AAA_Window_W-15),19)
        if imgui.Button("Delete Preference File",(ECC_Preferences.AAA_Window_W-15),20) then ECC_File_Delete("PrefsFile") ECC_Initialized = false end
